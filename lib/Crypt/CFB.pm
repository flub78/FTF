...
$VERSION = 0.01;
#
# Modified to make it compatible with Java and Crypt::OpenSSL::AES
#
...
sub _reginit {
	...
	return pack "NNNN", (0, 0, 0, $cnt);
	# return unpack "a*" , (pack "NNNN", (0, 0, 0, $cnt));
...
sub _encrypt {
	...
	for ($i = 0; $i < $l; $i += $self->{bytes}) {
        ...
        # $out =  bencrypt ($self, (substr $string, $i, $self->{bytes}) , $d) . $out;
	}
	return $out;
}
...
sub reset {
	my ($self, $cnt) = @_;
	$self->{register} = _reginit($self, $cnt);
}
